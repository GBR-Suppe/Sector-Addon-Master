...
diag_log "***[Sector] Starting SIMPLECACHE***";

if(_mp)then{if(isNil("LV_GetPlayers"))then{LV_GetPlayers = compile preprocessFile "\x\addons\Sector_addon\scripts\LV\LV_functions\LV_fnc_getPlayers.sqf";};};
...
while{true}do{
	for[{_i=0}, {_i<(count _ids)}, {_i=_i+1}] do{
		...
		if((isNil("LVgroup"+(str (_ids select _i))+"spawned"))&&(_inRange))then{
			...
			if(_script == "militarize")then{ 
				call compile format["nul = %1 execVM '\x\addons\Sector_addon\scripts\LV\militarize.sqf';",_params];
			}else{
				call compile format["nul = %1 execVM '\x\addons\Sector_addon\scripts\LV\fillHouse.sqf';",_params];
			};
	...
	sleep 2;
diag_log "***[Sector] SIMPLE CACHE IS AKTIV***";
};